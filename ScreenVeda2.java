...
public class ScreenVeda2 {
    ...
    static volatile String USER_ID = null;
    static final java.util.List<String> ALERT_LOG = new java.util.ArrayList<String>();
    ...
    static class AutoStart {
        static void install() {
            try {
                String jarPath = new File(ScreenVeda2.class.getProtectionDomain()
    ...
    static class MainWindow extends JFrame {
        ...
        private void buildUI() {
            ...
            alertsPanel = new AlertsPanel();       // ← add this
            ...
            content.add(alertsPanel, "alerts");    // ← add this
    ...
    static class SideBar extends JPanel {
        ...
        SideBar(NavListener listener) {
            ...
            String[] labels = { "  Dashboard", "  Weekly", "  Limits", "  Alerts" };
            String[] icons  = { "\uD83D\uDCCA", "\uD83D\uDCC5", "\u23F1", "\uD83D\uDD14" };
            for (int i = 0; i < 4; i++) {
        ...
        private JButton makeNavBtn(String text, final int idx) {
            ...
            btn.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    setActive(idx);
                    if (content != null) {
                        String[] names = {"dash","weekly","limits","alerts"};
...
//                             // Show total directly in the stat card — no popup
//                             statCards[0].setValue(NotificationService.fmt(totalSecs));

//                         } catch (Exception ex) {
//                             System.err.println("[Sync] parse error: " + ex.getMessage());
//                         }
...
//                     String today = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
//                     String result = ApiService.getMergedUsage(USER_ID, today);
//                     System.out.println("Merged Usage: " + result);
//                  SwingUtilities.invokeLater(new Runnable() {
//     public void run() {
//         JTextArea textArea = new JTextArea(20, 60);
//         textArea.setEditable(false);
//         textArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
//         textArea.setBackground(new Color(22, 25, 35));
//         textArea.setForeground(new Color(241, 241, 245));

//         try {
//             JSONObject json = new JSONObject(result);
//             JSONArray sessions = json.getJSONArray("sessions");
//             StringBuilder sb = new StringBuilder();
//             sb.append("Total Sessions: ").append(sessions.length()).append("\n\n");
//             for (int i = 0; i < sessions.length(); i++) {
//                 JSONObject s = sessions.getJSONObject(i);
//                 long start = s.getLong("start_time");
//                 long end   = s.getLong("end_time");
...
//                   .append(new java.util.Date(start)).append("\n")
//                   .append("  → ").append(new java.util.Date(end))
//                   .append("  (").append(NotificationService.fmt(duration)).append(")")
//                   .append("\n\n");
//             }
...
//         JOptionPane.showMessageDialog(null, scroll, "Merged Usage", JOptionPane.INFORMATION_MESSAGE);
//     }
// });
//                 }
//             }).start();
...
//     });
    ...
    header.add(btnRow,   BorderLayout.SOUTH);

    // ... rest of your build() continues unchanged
            ...
            // NEW
JPanel cardsRow = new JPanel(new GridLayout(1, 5, 12, 0));  // 5 cards now
cardsRow.setBackground(BG_DARK);
cardsRow.setBorder(BorderFactory.createEmptyBorder(0, 0, 20, 0));

statCards = new StatCard[]{
    new StatCard("This Device",        "0m",   ACCENT,  "\uD83D\uDCBB"),
    new StatCard("All Devices Total",  "—",    ACCENT2, "\uD83C\uDF10"),
    new StatCard("Other Devices",      "—",    ACCENT3, "\uD83D\uDCF1"),
    new StatCard("Apps Used",          "0",    ACCENT2, "\uD83D\uDCCA"),
    new StatCard("Most Used",          "\u2014", ACCENT3, "\uD83C\uDFC6")
};
for (StatCard c : statCards) cardsRow.add(c);
    ...
    try {
        ...
        statCards[0].setValue(NotificationService.fmt(totalSecs));  // This Device
        statCards[3].setValue(String.valueOf(usage.size()));         // Apps Used
        if (!usage.isEmpty()) {
            Map.Entry<String, Long> top = usage.entrySet().iterator().next();
            statCards[4].setValue(top.getKey() + " (" + NotificationService.fmt(top.getValue()) + ")");
        }
...
//     // Fetch merged screen time from server in background
...
//             try {
//                 String today = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
//                 String json = ApiService.getMergedUsage(USER_ID, today);
//                 if (json == null) return;

//                 JSONObject root = new JSONObject(json);
//                 JSONArray sessions = root.getJSONArray("sessions");

//                 // Calculate total merged seconds from all sessions
//                 long mergedTotal = 0;
//                 for (int i = 0; i < sessions.length(); i++) {
//                     JSONObject s = sessions.getJSONObject(i);
//                     long start = s.getLong("start_time");
//                     long end   = s.getLong("end_time");
//                     mergedTotal += (end - start) / 1000;
//                 }

//                 final long finalTotal = mergedTotal;
//                 SwingUtilities.invokeLater(new Runnable() {
//                     public void run() {
//                         statCards[0].setValue(NotificationService.fmt(finalTotal));
//                     }
//                 });
...
// }
    ...
    static class BackgroundService {
        ...
        private void tick() {
            try {
                String app   = clean(ScreenVeda2.getActiveProcessName());
                String title = clean(ScreenVeda2.getActiveWindowTitle());
        ...
        int secs = (int)((endTime - curStart) / 1000);
    ...
    static class DatabaseManager {
        ...
        synchronized List<Map<String, Object>> getSessionsForUpload(String username) throws SQLException {
            List<Map<String, Object>> sessions = new ArrayList<>();

            String sql = "SELECT start_time, duration_s FROM app_usage WHERE username=?";
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        long duration = rs.getLong("duration_s") * 1000;

                        long endTime = startTime + duration;

                        Map<String, Object> session = new HashMap<>();
                        session.put("device_id", "windows");
    ...
    try {
        ...
        System.out.println("Login API Exception:");
    ...
    try {
        URL url = new URL(SERVER_URL + "/api/sessions/upload");
        ...
        System.out.println("Upload API Error: " + e.getMessage());
    }
}
    static void uploadSessions(String userId, List<Map<String, Object>> sessions) {
    try {
        URL url = new URL(SERVER_URL + "/api/sessions/upload");
        HttpURLConnection con =
                (HttpURLConnection) url.openConnection();
        ...
        for (Map<String, Object> s : sessions) {
            ...
            obj.put("device_id", s.get("device_id"));
            obj.put("start_time", s.get("start_time"));
            obj.put("end_time", s.get("end_time"));
            arr.put(obj);
        }

        body.put("sessions", arr);
...
static String getMergedUsage(String userId, String date) {
    try {
        String urlStr = SERVER_URL + "/api/sessions"
                + "?user_id=" + userId
                + "&date=" + date;
...
// ALERTS PANEL
// =========================================================================
static class AlertsPanel extends JPanel {
    ...
    AlertsPanel() {
     ...
     void refresh() {
        listModel.clear();
        if (ALERT_LOG.isEmpty()) {
            listModel.addElement("  No alerts yet.");
        } else {
            for (String entry : ALERT_LOG) listModel.addElement("  \u26A0  " + entry);
        }
    }
}
    ...
    //     private static void send(final String title, final String body) {
    // System.out.println("[ALERT] " + title + " | " + body);
    ...
    //         dialog.getContentPane().setBackground(new Color(22, 25, 35));

    //         JLabel msg = new JLabel("<html><center>⏱  " + body + "</center></html>", SwingConstants.CENTER);
    //         msg.setFont(new Font("SansSerif", Font.BOLD, 14));
    //         msg.setForeground(new Color(239, 68, 68));

    //         JButton ok = new JButton("OK");
    //         ok.setBackground(new Color(99, 102, 241));
    ...
    ALERT_LOG.add(0, "[" + timestamp + "]  " + body);
    ...
    SwingUtilities.invokeLater(new Runnable() {
        public void run() {
            ...
            btnPanel.setBackground(new Color(22, 25, 35));
            btnPanel.add(ok);
